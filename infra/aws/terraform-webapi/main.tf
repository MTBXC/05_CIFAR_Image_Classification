...
resource "aws_ecr_repository" "webapi" {
  name                 = "${local.project_name}-webapi"
...
resource "aws_ecr_lifecycle_policy" "webapi" {
...
# Policy for ECS Task to access S3 (for model downloads)
    ...
    Statement = [
      {
        ...
        Resource = [
          "arn:aws:s3:::cifar-sagemaker-models-*",
          "arn:aws:s3:::cifar-sagemaker-models-*/*",
...
resource "aws_cloudwatch_log_group" "webapi" {
  name              = "/ecs/${local.project_name}-webapi"
  retention_in_days = 7
...
resource "aws_ecs_task_definition" "webapi" {
  family                   = "${local.project_name}-webapi"
  network_mode             = "awsvpc"
  ...
  container_definitions = jsonencode([
    {
      name  = "webapi"
      image = var.docker_image_uri != "" ? var.docker_image_uri : "${aws_ecr_repository.webapi.repository_url}:latest"
...
resource "aws_ecs_service" "webapi" {
  name            = "${local.project_name}-webapi-service"
  ...
  depends_on = [aws_lb_listener.webapi]
...
resource "aws_appautoscaling_target" "ecs_target" {
  ...
  resource_id        = "service/${aws_ecs_cluster.main.name}/${aws_ecs_service.webapi.name}"
